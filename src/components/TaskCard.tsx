...
export default function TaskCard({ task, completed = false, lastCompletion, onComplete }: Props) {
  ...
  return (
    <div
      className={`relative bg-white rounded-2xl border border-gray-100 p-4 flex items-center gap-3 shadow-sm transition-opacity ${
      ...
      <div className="flex-1 min-w-0">
        ...
        {completed && lastCompletion && (
          <p className="text-xs text-gray-400 mt-0.5">
            Erledigt am{' '}
            {new Date(lastCompletion.completedAt).toLocaleDateString('de-DE', {
              day: 'numeric',
              month: 'short',
            })}
          </p>
